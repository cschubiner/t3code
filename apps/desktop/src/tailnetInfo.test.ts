...
describe("readTailnetInfo", () => {
  ...
  it("parses a connected status with DNS + IPv4", async () => {
    const info = await readTailnetInfo(() =>
      Promise.resolve(
        JSON.stringify({
  ...
  it("reports disconnected when BackendState is Stopped and no tailnet IPs", async () => {
    const info = await readTailnetInfo(() =>
      Promise.resolve(
        JSON.stringify({
          BackendState: "Stopped",
          Self: { DNSName: "offline.ts.net.", Online: false, TailscaleIPs: [] },
        }),
      ),
    );
    expect(info.connected).toBe(false);
