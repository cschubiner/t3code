...
function persistenceErrorMessage(error: { readonly message: string } | unknown): string {
    ...
    return WsRpcGroup.of({
      ...
      [WS_METHODS.skillsSearch]: (input) =>
        Effect.tryPromise({
          ...
          catch: (cause) => ({ message: `Failed to search skills: ${String(cause)}` }),
        }),
      [WS_METHODS.snippetsList]: (_input) =>
        snippetRepository.listAll().pipe(
          Effect.map((snippets) => ({ snippets })),
          Effect.mapError((cause) => ({ message: persistenceErrorMessage(cause) })),
        ),
      [WS_METHODS.snippetsCreate]: (input) =>
        snippetRepository
          ...
          .pipe(Effect.mapError((cause) => ({ message: persistenceErrorMessage(cause) }))),
      ...
      [WS_METHODS.subscribeSnippetsUpdated]: (_input) => snippetRepository.streamChanges,
    });
  }),
);
