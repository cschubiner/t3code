...
import { Config, Effect, Layer, LogLevel, Option, Schema } from "effect";
...
const rootCommand = Command.make("t3", commandFlags).pipe(
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      const serverRuntimeLayer = Layer.empty.pipe(
        Layer.provideMerge(ServerSettingsLive),
        Layer.provideMerge(SqlitePersistenceLayerLive),
        Layer.provideMerge(Layer.succeed(ServerConfig, config)),
      );
      return yield* runServer.pipe(Effect.provide(serverRuntimeLayer));
    }),
  ),
);
