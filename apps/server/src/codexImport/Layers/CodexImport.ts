...
function readThreadRows(databasePath: string): Array<CodexThreadRow> {
  ...
  try {
    ...
    `);
    return (statement.all() as unknown as Array<RawCodexThreadRow>).map(normalizeThreadRow);
  } finally {
    database.close();
  }
}
...
function toSummary(
  ...
  const importedThread = readModel.threads.find((thread) => thread.id === importedThreadId);
  ...
  return {
    ...
    alreadyImported: importedThread !== undefined,
    importedThreadId: importedThread?.id ?? null,
...
const makeCodexImport = Effect.gen(function* () {
  ...
  const inferCanonicalWorkspaceFromReadModel = (
    ...
    Effect.sync(() => {
      ...
      const candidates = readModel.projects.filter(
        (project) =>
          project.deletedAt === null &&
          !isCodexManagedWorktreePath(project.workspaceRoot) &&
          path.basename(project.workspaceRoot) === repoName,
      );
    ...
    Effect.gen(function* () {
      ...
      for (const [sourceProjectId, targetProjectId] of duplicateProjectTargets.entries()) {
        const threadsToMove = latestSnapshot.threads.filter(
          (thread) => thread.projectId === sourceProjectId && thread.deletedAt === null,
        );
