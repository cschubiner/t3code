...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("maps blocked-by-policy Codex error notifications to runtime.warning", () =>
    Effect.gen(function* () {
      const adapter = yield* CodexAdapter;
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);

      lifecycleManager.emit("event", {
        id: asEventId("evt-policy-blocked-error"),
        kind: "notification",
        provider: "codex",
        threadId: asThreadId("thread-1"),
        createdAt: new Date().toISOString(),
        ...
        payload: {
          error: {
            message:
              'exec_command failed: CreateProcess { message: "Rejected(\\"command rejected: blocked by policy\\")" }',
          },
        },
      } satisfies ProviderEvent);
      ...
      assert.equal(firstEvent.value.turnId, "turn-1");
      assert.match(firstEvent.value.payload.message, /blocked by policy/i);
    }),
  );

  it.effect("maps closed-stdin process stderr errors to runtime.warning", () =>
    Effect.gen(function* () {
      const adapter = yield* CodexAdapter;
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);

      lifecycleManager.emit("event", {
        id: asEventId("evt-closed-stdin-error"),
        kind: "error",
        provider: "codex",
        threadId: asThreadId("thread-1"),
        createdAt: new Date().toISOString(),
        ...
        turnId: asTurnId("turn-1"),
        message:
          "2026-03-23T17:23:03.336565Z ERROR codex_core::tools::router: error=write_stdin failed: stdin is closed for this session; rerun exec_command with tty=true to keep stdin open",
      } satisfies ProviderEvent);
      ...
      assert.equal(firstEvent.value.turnId, "turn-1");
      assert.match(firstEvent.value.payload.message, /stdin is closed/i);
    }),
  );

  it.effect("maps models cache ttl stderr errors to runtime.warning", () =>
    Effect.gen(function* () {
      const adapter = yield* CodexAdapter;
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);

      lifecycleManager.emit("event", {
        id: asEventId("evt-cache-ttl-error"),
        kind: "error",
        provider: "codex",
        threadId: asThreadId("thread-1"),
        createdAt: new Date().toISOString(),
        ...
        turnId: asTurnId("turn-1"),
        message:
          "2026-03-23T17:25:44.123550Z ERROR codex_core::models_manager::manager: failed to renew cache TTL: EOF while parsing a value at line 1 column 0",
      } satisfies ProviderEvent);
      ...
      assert.equal(firstEvent.value.turnId, "turn-1");
      assert.match(firstEvent.value.payload.message, /failed to renew cache ttl/i);
    }),
  );
  ...
  it.effect("maps auth-required stderr notifications to runtime.error", () =>
    Effect.gen(function* () {
      ...
      lifecycleManager.emit("event", {
        id: asEventId("evt-process-stderr-auth-required"),
        kind: "notification",
        provider: "codex",
        threadId: asThreadId("thread-1"),
        createdAt: new Date().toISOString(),
        ...
        turnId: asTurnId("turn-1"),
        message:
          '2026-04-02T00:37:24.039974Z ERROR rmcp::transport::worker: worker quit with fatal: Transport channel closed, when AuthRequired(AuthRequiredError { www_authenticate_header: "Bearer realm=\\"OpenAI API\\"" })',
      } satisfies ProviderEvent);
