...
type LegacyFlatSettings = {
  ...
  customClaudeModels: ReadonlyArray<string>;
  customCodexModels: ReadonlyArray<string>;
};

type CompatibilitySettings = UnifiedSettings & LegacyFlatSettings;

function splitPatch(patch: Partial<CompatibilitySettings>): {
  ...
  for (const [key, value] of Object.entries(patch)) {
    if (key === "codexBinaryPath") {
      serverPatch.providers ??= {};
      (serverPatch.providers as Record<string, unknown>).codex = {
        ...((serverPatch.providers as Record<string, unknown>).codex as Record<string, unknown>),
        binaryPath: value,
      };
      continue;
    }
    if (key === "codexHomePath") {
      serverPatch.providers ??= {};
      (serverPatch.providers as Record<string, unknown>).codex = {
        ...((serverPatch.providers as Record<string, unknown>).codex as Record<string, unknown>),
        homePath: value,
    ...
    if (key === "claudeBinaryPath") {
      serverPatch.providers ??= {};
      (serverPatch.providers as Record<string, unknown>).claudeAgent = {
        ...((serverPatch.providers as Record<string, unknown>).claudeAgent as Record<
          string,
          unknown
        >),
        binaryPath: value,
    ...
    if (key === "customCodexModels") {
      serverPatch.providers ??= {};
      (serverPatch.providers as Record<string, unknown>).codex = {
        ...((serverPatch.providers as Record<string, unknown>).codex as Record<string, unknown>),
        customModels: normalizeCustomModelSlugs(
    ...
    if (key === "extraSkillRoots") {
      clientPatch.extraSkillRoots = normalizeSkillRootPaths(
        Array.isArray(value) ? value : [],
      ) as ClientSettings["extraSkillRoots"];
      continue;
    }
...
export function useUpdateSettings() {
  ...
  const updateSettings = useCallback(
    (patch: Partial<CompatibilitySettings>) => {
...
export function buildLegacyClientSettingsMigrationPatch(
  ...
  if (Array.isArray(legacySettings.extraSkillRoots)) {
    patch.extraSkillRoots = normalizeSkillRootPaths(legacySettings.extraSkillRoots);
  }
