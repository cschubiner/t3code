...
function getProviderSummary(provider: ServerProvider | undefined): {
  ...
  if (!provider.enabled) {
    return {
      headline: "Disabled",
      detail:
        provider.message ?? "This provider is installed but disabled for new sessions in ClayCode.",
    };
  }
...
function SettingsRouteView() {
  ...
  const isInstallSettingsDirty =
    settings.claudeBinaryPath !== DEFAULT_UNIFIED_SETTINGS.providers.claudeAgent.binaryPath ||
    settings.codexBinaryPath !== DEFAULT_UNIFIED_SETTINGS.providers.codex.binaryPath ||
    settings.codexHomePath !== DEFAULT_UNIFIED_SETTINGS.providers.codex.homePath;
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="mx-auto flex w-full max-w-2xl flex-col gap-6">
            {isElectron ? (
              <section className="rounded-2xl border border-border bg-card p-5">
                <div className="mb-4 flex items-start justify-between gap-3">
                  ...
                  <div className="rounded-full border border-sky-500/30 bg-sky-500/10 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-sky-700">
                    Tailscale
                  </div>
                </div>
            ...
            <SettingsSection title="General">
              <SettingsRow
                title="Theme"
                description="Choose how ClayCode looks across the app."
                resetAction={
                  theme !== "system" ? (
