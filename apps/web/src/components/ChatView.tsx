...
Object.freeze(EMPTY_QUEUED_COMPOSER_TURNS);
...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const takeQueuedTurn = useComposerDraftStore((store) => store.takeQueuedTurn);
  const insertQueuedTurnAt = useComposerDraftStore((store) => store.insertQueuedTurnAt);
  const updateQueuedTurn = useComposerDraftStore((store) => store.updateQueuedTurn);
  const moveQueuedTurn = useComposerDraftStore((store) => store.moveQueuedTurn);
  ...
  const queuedTurnDispatchRef = useRef<{ queuedTurnId: string; source: "auto" | "manual" } | null>(
    null,
  );
  const dragDepthRef = useRef(0);
  const terminalOpenByThreadRef = useRef<Record<string, boolean>>({});
  const [queuedTurnDispatch, setQueuedTurnDispatch] = useState<{
    queuedTurnId: string;
    source: "auto" | "manual";
  } | null>(null);
  ...
  const composerHasContent = prompt.trim().length > 0 || composerImages.length > 0;
  ...
  const queueComposerTurn = useCallback(
    async (input?: { text: string; interactionMode: ProviderInteractionMode }) => {
      ...
      if (!trimmed && queuedImages.length === 0) {
        return false;
      }
  ...
  const flushQueuedTurn = useCallback(
    async (input: {
      ...
      if (queuedTurnDispatchRef.current !== null) {
        return false;
      }

      const dispatchState = {
        queuedTurnId: input.turn.id,
        source: input.source,
      } as const;
      queuedTurnDispatchRef.current = dispatchState;
      setQueuedTurnDispatch(dispatchState);

      try {
        const dispatched = await dispatchExistingThreadTurn({
          ...
          modelOptions: input.turn.modelOptions?.codex
            ? {
                codex: {
                  ...(input.turn.modelOptions.codex.reasoningEffort !== undefined
                    ? {
                        reasoningEffort: input.turn.modelOptions.codex.reasoningEffort,
                      }
                    : {}),
                  ...(input.turn.modelOptions.codex.fastMode === true ? { fastMode: true } : {}),
                },
              }
            : null,
        ...
        if (!dispatched) {
          insertQueuedTurnAt(threadId, input.restoreIndex, input.turn);
          return false;
        }
        return true;
      } catch {
        ...
        if (queuedTurnDispatchRef.current?.queuedTurnId === input.turn.id) {
          queuedTurnDispatchRef.current = null;
        }
        setQueuedTurnDispatch((current) =>
          current?.queuedTurnId === input.turn.id ? null : current,
        );
      }
    },
    [dispatchExistingThreadTurn, insertQueuedTurnAt, threadId],
  );

  const handleSaveQueuedTurnEdit = useCallback(
    (queuedTurnId: string, text: string) => {
      const queuedTurn = queuedTurns.find((turn) => turn.id === queuedTurnId);
      if (!queuedTurn) {
        return;
      }
      const trimmed = text.trim();
      if (!trimmed && queuedTurn.images.length === 0) {
        return;
      }
      updateQueuedTurn(threadId, queuedTurnId, (turn) => ({
        ...turn,
        text: trimmed,
      }));
    },
    [queuedTurns, threadId, updateQueuedTurn],
  );

  const handleReorderQueuedTurns = useCallback(
    (activeQueuedTurnId: string, targetQueuedTurnId: string) => {
      if (queuedTurnDispatchRef.current !== null) {
        return;
      }
      moveQueuedTurn(threadId, activeQueuedTurnId, targetQueuedTurnId);
    },
    [moveQueuedTurn, threadId],
  );

  const handleSendQueuedTurnNow = useCallback(
    async (queuedTurnId: string) => {
      if (queuedTurnDispatchRef.current !== null) {
        return;
      }
      const restoreIndex = queuedTurns.findIndex((turn) => turn.id === queuedTurnId);
      if (restoreIndex < 0) {
        return;
      }
      const turn = takeQueuedTurn(threadId, queuedTurnId);
      if (!turn) {
        return;
      }
    ...
    [flushQueuedTurn, queuedTurns, takeQueuedTurn, threadId],
  );
  ...
  useEffect(() => {
    ...
    const turnToFlush = takeQueuedTurn(threadId, nextQueuedTurn.id);
    if (!turnToFlush) {
      return;
    }
          ...
          <div className={cn("px-3 pt-1.5 sm:px-5 sm:pt-2", isGitRepo ? "pb-1" : "pb-3 sm:pb-4")}>
            ...
            >
              <QueuedFollowUpsPanel
                ...
                busyQueuedTurnId={queuedTurnDispatch?.queuedTurnId ?? null}
                isQueueInteractionDisabled={queuedTurnDispatch !== null || isConnecting}
                onDelete={(queuedTurnId) => removeQueuedTurn(threadId, queuedTurnId)}
                onClearAll={() => clearQueuedTurns(threadId)}
                onSaveEdit={handleSaveQueuedTurnEdit}
                onSendNow={(queuedTurnId) => void handleSendQueuedTurnNow(queuedTurnId)}
                onReorder={handleReorderQueuedTurns}
              />
